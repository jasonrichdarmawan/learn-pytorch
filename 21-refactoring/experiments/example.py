...
class FileL1:
    ...
    def __repr__(self):
        return f"FileL1({self.name=}, {self.size=})"
...
class FileHostingServiceL1(Generic[FL1, FF1]):
    ...
    def _resolve_path(self, file_name: str) -> tuple[Directory[FL1], str]:
        ...
        for part in dir_parts:
            if part not in current_dir.subdirectories:
                current_dir.subdirectories[part] = Directory[FL1](name=part)
    ...
    def _check_pre_upload(self, current_dir: Directory[FL1], file_part: str) -> None:
        if file_part in current_dir.files:
            raise RuntimeError(
                f"File {current_dir.name}/{file_part} already exists (L1 check)."
            )
    ...
    def file_upload(self, file_name: str, size: int):
        ...
        current_dir, file_part = self._resolve_path(file_name=file_name)

        # If a file with the same name already exists on the server, throws a runtime exception
        self._check_pre_upload(current_dir=current_dir, file_part=file_part)

        file_obj = self._file_factory.__call__(file_name=file_part, size=size)

        # Upload the file to the remote storage server
        self._store_file(current_dir=current_dir, file_part=file_part, file=file_obj)
        print(f"Uploaded {file_name} ({file_obj.size} bytes)")
    ...
    def _check_pre_copy(
        self,
        source_dir: Directory[FL1],
        source_part: str,
        dest_dir: Directory[FL1],
        dest_part: str,
    ):
...
class FileHostingServiceL2(FileHostingServiceL1[FL1, FF1]):
    ...
    def file_search(self, prefix: str) -> list[FL1]:
        ...
        matching_files.sort(key=lambda file: (-file.size, file.name))
...
if MAIN:
    ...
    assert len(service.file_search("file")) == 10
    assert len(service.file_search("filt")) == 0
    assert len(service.file_search("a")) == 1
    assert len(service.file_search("b")) == 1
    assert len(service.file_search("c")) == 0
...
class FileL2(FileL1):
    ...
    but also with an additional parameter
    to include a timestamp for the operation,
    ...
    def __repr__(self):
        return f"FileL2({self.name=}, {self.size=}, {self.timestamp=}, {self.ttl=})"
...
class FileFactoryL2(FileFactory[FileL2]):
    def __call__(
        ...
        print(
            f"Creating FileL2 instance: {name} ({size} bytes) with timestamp {timestamp} and ttl {ttl}"
        )
        return FileL2(name=name, size=size, timestamp=timestamp, ttl=ttl)
...
class FileHostingServiceL3(FileHostingServiceL2[FL2, FF2]):
    ...
    def file_upload_at(
        ...
        """
        - If ttl is provided, the uploaded file is available
        for ttl seconds.
        """
        current_dir, file_part = self._resolve_path(file_name)
        ...
        file_obj = self._file_factory.__call__(
            name=file_part, size=file_size, timestamp=timestamp, ttl=ttl
        )
    ...
    def _check_pre_copy_l3(
        self,
        timestamp: float,
        source_dir: Directory[FL2],
        source_part: str,
        dest_dir: Directory[FL2],
        dest_part: str,
    ):
        self._check_pre_copy(source_dir, source_part, dest_dir, dest_part)
    ...
    def file_copy_at(self, timestamp: float, file_from: str, file_to: str):
        ...
        source_file = source_dir.files[source_part]
        copied_file = self._file_factory.__call__(
            name=dest_part,
...
if MAIN:
    ...
    assert service.file_get_at(timestamp=2, file_name="file-1-copy.zip") == None
    ...
    assert len(service.file_search_at(timestamp=2, prefix="file")) == 10
    assert len(service.file_search_at(timestamp=3, prefix="file")) == 0
    assert len(service.file_search_at(timestamp=2, prefix="filt")) == 0
...
class FileHostingServiceL4(FileHostingServiceL3[FL2, FF2]):
    ...
    def rollback(self, timestamp: float):
        """
        - Rollback the state of the file storage to the state specified
        in the timestamp
        ...
        def recursive_rollback(current_dir: Directory[FL2]):
            ...
            for _, subdir in current_dir.subdirectories.items():
                recursive_rollback(current_dir=subdir)
...
if MAIN:
    ...
    service.file_upload_at(
        timestamp=1, file_name="dir-a/file-1.txt", file_size=2, ttl=1
    )
    service.file_upload_at(
        timestamp=2, file_name="dir-b/file-2.txt", file_size=1, ttl=1
    )
