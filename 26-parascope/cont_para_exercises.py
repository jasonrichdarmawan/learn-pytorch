...
# %%
WORKSPACE = "/media/npu-tao/disk4T/jason/transformers"
...
class ActStore:
  def __init__(self, model: HookedTransformer):
    ...
    self.chosen_hook_list = [f'blocks.{i}.{h}' for h in chosen_hooks for i in Ls]
...
def transfer_activations(tokens: torch.Tensor, num_copies: int = 10, num_tokens: int = 30):
  ...
  with model.hooks(fwd_hooks=act_store.store_hook_list()):
    ...
    print(f"tensor.shape: {tokens.shape}, logits.shape: {logits.shape}")
...
# %% [markdown]
# What do you see?
# You should see that the transfered activations 
# give *some* indicaiton of what the model was 
# planning to write, but that they are far from 
# perfect. It seems like the model can guess 
# the next few words, and can sometimes bootstrap 
# from there, but often fails to.
# Can we evaluate this more automatically?

# %% [markdown]
# # Measuring the quality of the continuations.
# We use semantic text embeddings to measure 
# how well our continuation ParaScope technique 
# works.
# Semantic embeddings convert text into 
# high-dimensional vectors that 
# capture meaning;
# texts with
# similar meanings will have vectors that point 
# in similar directions (high cosine similarity).
#
# Our evaluation process:
# 1. Encode both the original paragraph and 
# the generated continuation into embedding vectors
# 2. Calculate cosine similarity between 
# these vectors (ranges from -1 to 1, 
# where 1 = identical meaning)
# 3. Higher similarity scores indicate that 
# the continuation successfully captured the 
# semantic content that the model was "planning" 
# to write in the original context
#
# This helps us quantify whether activation
# transfer preserves meaningful contextual 
# information
# or just produces random text that happens 
...
def calculate_similarities(
  ...
  TODO: Why encode the original text into embeddings? 
  e.g. "The quick brown fox jumps over the lazy dog" into torch.Tensor with shape (768)
  """
  similarities = []
  ...
  orig_embedding = sentence_model.encode(
    original_text,
  ...
  for cont_tokens in continued_tokens:
    ...
    if cont_text:
      cont_embedding = sentence_model.encode(
        cont_text,
        convert_to_tensor=True,
      )

      cosine_sim = torch.cosine_similarity(
        x1=orig_embedding.unsqueeze(0),
        x2=cont_embedding.unsqueeze(0),
      )
...
par1_similarities = calculate_similarities(
  original_text=par1_str,
  continued_tokens=par1_continued_tokens,
)
par2_similarities = calculate_similarities(
  original_text=par2_str, 
  continued_tokens=par2_continued_tokens,
)
