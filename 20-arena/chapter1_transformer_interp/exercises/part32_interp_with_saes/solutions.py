...
if MAIN:
    print(
        tabulate(
            gpt2_sae.cfg.__dict__.items(), headers=["name", "value"], tablefmt="simple_outline"
        )
    )
...
def get_k_largest_indices(
    ...
    assert not no_overlap or k <= x.size(
        0
...
if MAIN:
    ...
    for i in tqdm(
        range(total_batches), desc="Computing activations data for PCA, over all batches"
    ):
        _, cache = gpt2.run_with_cache_with_saes(
            tokens := gpt2_act_store.get_batch_tokens(),
            saes=[gpt2_sae],
...
if MAIN:
    ...
    seq_len = (
        10  # higher seq len / more batches would be more reliable, but this simplifies the plot
    )
...
if MAIN:
    if USING_GEMMA:
        ...
        print(
            tabulate(
                gemma_2b_sae.cfg.__dict__.items(),
                headers=["name", "value"],
                tablefmt="simple_outline",
            )
        )
