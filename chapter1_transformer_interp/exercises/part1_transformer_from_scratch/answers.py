...
def rand_float_test(cls: nn.Module, shape: list[int]):
...
def rand_int_test(cls: nn.Module, shape: list[int]):
    cfg = Config(debug=True)
    layer = cls(cfg).to(device)
...
def load_gpt2_test(cls: nn.Module, gpt2_layer: nn.Module, input: Tensor):
    cfg = Config(debug=True)
    layer: nn.Module = cls(cfg).to(device)
...
class LayerNorm(nn.Module):
    ...
    def forward(self, residual: Float[Tensor, "batch posn d_model"]) -> Float[Tensor, "batch posn d_model"]:
        ...
        # Centering: This process involves subtracting the mean from each data point 
        # so that the dataset has a mean of zero.
        # It shifts the distribution of data without changing its shape.
        # Scaling: This process invovles dividing (or multiplying) the data poitns
        # by a measure of spread (like the standard deviation).
        # Scaling adjusts the range or variance of the data, often standardizing it
...
class Attention(nn.Module):
    ...
    def forward(self, normalized_resid_pre: Float[Tensor, "batch posn d_model"]) -> Float[Tensor, "batch posn d_model"]:
        ...
        # Here, the d_model dimension is the common axis between the two tensors.
        # It's being summed over (contracted) in the multiplication.
        ...
        # The operation performs a sum over the common dimension d_head. That is, for each batch
        # element, for each head, and for every pair of query and key positions, it computes:
        # attn_scores[i, h, pq, pk] = \sum^{d_head}_{d=1} q[i, pQ, h, d] * k[i, pK, h, d]
        ...
        attn_out = einops.einsum(z, self.W_O,
                                 "batch nheads posn_Q d_head, nheads d_head d_model -> batch posn_Q d_model") + self.b_O
        return attn_out
...
class MLP(nn.Module):
    ...
    def forward(self, normalized_resid_mid: Float[Tensor, "batch posn d_model"]) -> Float[Tensor, "batch posn d_model"]:
        ...
        post = gelu_new(pre)
        mlp_out = einops.einsum(post, self.W_out,
...
def get_log_probs(
    logits: Float[Tensor, "batch posn d_vocab"], tokens: Int[Tensor, "batch posn"]) -> Float[Tensor, "batch posn-1"]:
    log_probs = logits.log_softmax(dim=-1)
    # Get logprobs the first seq_len-1 predictions 
    # (so we can compare them with the actual next tokens)
...
if MAIN:
    ...
    train_loader = DataLoader(
        dataset_dict["train"], batch_size=args.batch_size, shuffle=True, 
        num_workers=4, pin_memory=True
    )
    test_loader = DataLoader(
        dataset_dict["test"], batch_size=args.batch_size, shuffle=False, 
        num_workers=4, pin_memory=True
    )
...
class TransformerTrainer(Generic[GenericArgs]):
    def __init__(self, args: GenericArgs, model: DemoTransformer):
        ...
        self.optimizer = t.optim.AdamW(self.model.parameters(), lr=args.lr, 
                                       weight_decay=args.weight_decay)
        self.step = 0

        self.train_loader = DataLoader(
            dataset_dict["train"], batch_size=args.batch_size, shuffle=True, num_workers=4, 
            pin_memory=True
        )
...
class TransformerSampler:
    ...
    def sample_top_k(logits: Float[Tensor, "d_vocab"], k: int) -> int:
        ...
        """
        top_k_logits, top_k_token_ids = t.topk(logits, k)
    ...
    def sample_top_p(logits: Float[Tensor, "d_vocab"], top_p: float, min_tokens_to_keep: int = 1) -> int:
        ...
        cumul_probs = logits_sorted.softmax(dim=-1).cumsum(dim=-1)
...
if MAIN:
    ...
    # apply_frequency_penalty logits is t.ones
...
class Beams:
    ...
    def generate(self, k: int, no_repeat_ngram_size: int | None = None) -> "Beams":
        ...
        # Add new logprobs & concat new tokens. When doing this, we need to add an extra `k` dimension
        # since our current logprobs & tokens have shape (batch,) and (batch, seq),
        # but our new ones bot have shape (batch, k)
        new_logprob_sums = einops.repeat(self.logprob_sums, "b -> b k", k=k) + topk_logprobs
        # cumulative logprobs for each beam
        # [a       a       a      ]   [kid child   little]
        # [in      in      in     ] + [the college high]
    ...
    def filter(self, k: int) -> tuple["Beams", "Beams"]:
        ...
        # Get the indices of the `k` best sequences (some termianted, some not terminated)
        ...
        # Return the beam objects fro mthee indices
    ...
    def get_topk_non_repeating(self, logprobs: Float[Tensor, "batch d_vocab"], 
                               no_repeat_ngram_size: int | None, k: int,
        ...
        if (no_repeat_ngram_size is not None) and (seq_len > no_repeat_ngram_size - 1):
            ...
            last_ngram_prefix = self.tokens[:, seq_len - (no_repeat_ngram_size - 1) :] # (batch, last ngram prefix)
