...
# ============================================
#
# While continuation parascopes directly use the 
# model's own generation capabilities,
# SONAR parascopes take a different approach by 
# learning to map residual streams to
...
#
# The SONAR approach:
# 1. Train a probe (Linear or MLP) to map from 
#    residual stream → SONAR embedding space
# 2. Use SONAR's decoder to convert embeddings back 
#    to text
# 3. This allows us to extract semantic content 
#    without relying on the model's generation
#
# Learning objectives:
# 1. Understand the SONAR text autoencoder and 
#    its embedding space
# 2. Load pre-generated datasets of residual 
#    streams and SONAR embeddings
# 3. Train probes to map between these spaces
# 4. Evaluate and compare with continuation 
#    parascopes
...
# %%
#!pip install sonar-space torch torchvision transformer-lens sentence-transformers
#
#!pip install matplotlib seaborn pandas numpy scikit-learn einops
...
#
# Since we want the probe to match "what the model 
# might output", we need data of what the model's 
# output might look like.
# Two-step process: (1) load dataset + generate 
# prompts, (2) use prompts to generate model 
# outputs.
...
for data in train_dataloader:  # Start with just 4 examples
  ...
  meta_prompt = format_question(text=data["text"])
  formatted_prompt = format_prompt(
    prompt=meta_prompt, 
    system_prompt="You are a prompt-writing assistant. You are given a text and you need to write a prompt that will generate a response that is similar to the text.",
  )
...
for i, prompt in enumerate(prompts):
  ...
  formatted_prompt = format_prompt(prompt=prompt)
  prompt_tokens = model.to_tokens(
    formatted_prompt, 
    prepend_bos=False, 
    padding_side="left",
...
from datasets import load_dataset
dataset = load_dataset(
  "nickypro/fineweb-llama3b-regen", 
  split="train"
)
...
# As we want to predict that the "next section" of 
# the text will say, we need to split the text 
# into sections.
# I choose to split the text by paragraphs, 
# but it is also reasonable to split it by 
...
def split_text_into_paragraphs(text: str) -> List[str]:
  ...
  The convention we choose is to have the newlines 
  stored at the end of each paragraph.
...
def split_dataset_prompt_and_sections(dataset) -> List[List[str]]:
  """
  For each example in the dataset, keeps the prompt 
  as a single string,
  ...
  for i, example in enumerate(tqdm(dataset)):
    prompt = format_prompt(prompt=example["prompt"])
    completion = example["completion"]
    completion_paragraphs = split_text_into_paragraphs(
      text=completion,
    )
...
split_sections = load_dataset(
  "nickypro/fineweb-llama3b-regen-split-formatted", 
  split="train",
)
...
# %% [markdown]
# ## Exercise 2: Loading Residual Streams and 
# Embeddings
#
...
# %% [markdown]
# We make hooks for storing activations of the 
# residual stream at the correct positions. 
# That is, the final token of each "section" of 
# tokens.
...
def get_act_data(
  ...
  all_tokens = torch.cat(
    [prompt_tokens, torch.cat(output_tokens, dim=1)], 
    dim=1
  )
...
act_data = get_act_data(
  split_text=split_sections[0]["split_text"], 
  verbose=True,
)
...
# Note these are slightly older files, 
# so these match up with a different dataset.

split_sections = load_dataset(
  "nickypro/llama-3b-split", 
  split="train",
)
print(split_sections[0])
...
# Now we use SONAR to create embeddings for each 
# paragraph.
# We use the `sonar.inference_pipelines.text.TextToEmbeddingModelPipeline` 
# class to create the embeddings.
...
# %% We try to get the embeddings, and also 
# compare some examples of how well it decodes.
...
for i, example in enumerate(tqdm(split_sections)):
  ...
  decoded_texts = vec2text.predict(
    embeddings, target_lang="eng_Latn",
  )
...
# %% alternatively, we can yet again use the 
...
# %% [markdown]
# Now we have shown how to get the data, 
# we can remove the text2vec pipeline from memory. 
# and instead use the pre-computed embeddings.
# %%
del text2vec
torch.cuda.empty_cache()
...
# - residuals[:-1]
# - embeds[1:]
# since we only use each residual 
# to predict the next embedding.
...
# So we can start training, right?

# %% [markdown]
# ## Exercise 3: Normalization and Preprocessing
#
# One issue with residual streams, is that the 
# magnitudes of the activations can vary a lot 
# between layers, often by orders of magnitude.
# This can cause issues for training, so we need 
# to normalize the data.
# We use Welford's algorithm to compute running 
# statistics.
# We compute the mean and variance of the residual 
# streams and embeddings, and then normalize the 
# data to have mean 0 and variance 1.
# We then store the mean and variance, so we can 
# restore the data later.
#
# In essense, we do the most naive method of 
# normalization, which is to look at each 
# dimension independently and normalize it to 
# have mean 0 and variance 1.
# There may be better ways do do this, I have not 
# spent much time optimizing this.
# While we do need to normalize the residuals, I 
# am not sure if we need to do it for the 
# embeddings, but I do it anyway.

# %%
@dataclass
class WelfordStats:
  """
  Track running mean and variance using 
  Welford's algorithm.
  """
  mean: torch.Tensor
  m2: torch.Tensor
  # `m2`` accumulates the sum of squares
  ...
  def update(
    ...
    new batch of data (batched version, 
    true Welford).
    """
    # new_data: (batch, d)
    ...
    for x in new_data:
      self.count += 1
      delta = x - self.mean
      # difference from the current mean
      self.mean += delta / self.count
      delta2 = x - self.mean
      # difference from the updated mean
...
# Update with data
for i, example in enumerate(tqdm(dataset)):
  res_stats.update(new_data=example["res_data"])
  embed_stats.update(new_data=example["embeds"])
...
def test_normalization(dataset):
  normalized_res    = res_normalizer.normalize(
    x=dataset[0]["res_data"]
  )
  normalized_embeds = embed_normalizer.normalize(
    x=dataset[0]["embeds"]
  )
...
#
# We now train the probe to map from 
# [residual stream] to [SONAR embedding].
# For efficiency, we currently load data from 
# 10,000 texts (index=0...9), but this could be 
# extended to 100,000 (index=0...99).
# We use index 99 as a relatively independent 
# validation set, and validate every epoch.
#
...
class ProbeTrainer:
  ...
  def preprocess_dataset(dataset: list[dict]):
    ...
    dataset_dict = {
      "texts": [
        example["split_text"][1:] 
        for example in dataset
      ],
      "res_data": res_normalizer.normalize(
        x=torch.cat(
          [
            example["res_data"] 
            for example in dataset
      ...
      "embeds": embed_normalizer.normalize(
        x=torch.cat(
          [
            example["embeds"] 
            for example in dataset
  ...
  def train_epoch(
    ...
    """
    Train for one epoch with improved memory 
    management.
    """
    self.probe.train()
    epoch_train_loss = 0
  ...
  def validate(self, val_indices: List[int]) -> float:
    """
    Validate the model with improved memory 
    management.
    """
    self.probe.eval()
    epoch_val_loss = 0
...
dataset_dict = ProbeTrainer.preprocess_dataset(
  load_all_data(99)
)

flat_texts = [section for example in dataset_dict["texts"] for section in example]

for i, (text, res, emb) in list(
  enumerate(
    zip(
      flat_texts,
      dataset_dict["res_data"], 
      dataset_dict["embeds"]
    )
  )
)[:100:5]:
